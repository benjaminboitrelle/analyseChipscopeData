...
std::vector<std::vector<int>> ReadChipscope::getADCInfo(std::vector<std::vector<int>> input, int adcPartBegin, int adcPartEnd, int bitsLength, int rowGroupDataLength){
  
  std::vector<std::vector<int>> adcDecimals;
  std::vector<std::vector<int>> adcBits(input.begin() + adcPartBegin, input.begin() + adcPartEnd);   
  for (unsigned int bit = 0; bit < (adcBits.size() / bitsLength); bit++){
    std::vector<int> tmpAdcDecimals;
    for (unsigned int column = 0; column < adcBits[bit].size(); column++){
      std::vector<int> pixelBits;
      for (auto numberOfBits = 0; numberOfBits < bitsLength; numberOfBits++){
        pixelBits.push_back(input[bit + (numberOfBits * rowGroupDataLength)][column]);
      }
      tmpAdcDecimals.push_back(convertBitsToDecimals(pixelBits));
    }  
    adcDecimals.push_back(tmpAdcDecimals);
  }
  return adcDecimals;
}
...
void ReadChipscope::readPrnFile(std::fstream& myFile, char delimiterChar, int beginData, int endData, std::vector<std::vector<int>> &outputImageVectorised){
  // Open a .prn file produced by Chipscope and store its element in a 2D vector of int with the appropriate bit coding

  std::string outputLine;
  myFile.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); // First line of .prn file contains comment
  while (std::getline(myFile, outputLine)){
    std::vector<std::string> result;
    splitString(outputLine, delimiterChar, result);
    std::vector<std::string> tmpResult(&result[beginData], &result[endData]); // H0_0 starts at the 3rd position and H1_21 is at the 47 position
    std::vector<int> resultIntFormat(tmpResult.size());
    convertStringToInt(tmpResult, resultIntFormat);
    //inverseBits(resultIntFormat);
    for (auto &bit : resultIntFormat) (bit == 0)? bit = 1 : bit = 0; // There is an inversion of bits in file. A bit 1 has to be considered as 0 and vice-versa
